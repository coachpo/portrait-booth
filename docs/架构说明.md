...
│   └── src/
│       ├── app/                  # 路由、布局、隐私页
│       ├── pages/                # home / create / retrieve / privacy / templates/:id
│       ├── create/               # 向导状态机与步骤组件
│       ├── camera/               # getUserMedia、session token、拍摄
│       ├── pose/                 # Face Landmarker 封装、姿态推导、质量检查
│       ├── editor/               # 非破坏性变换、蒙版、撤销
│       ├── render/               # 终态渲染、FinalArtifact、JPEG/PPI
...
│   │   ├── keygen.py             # KEY 生成
│   │   ├── hmac_utils.py         # 域隔离 HMAC
│   │   ├── image_validate.py     # 解码限制与重编码
│   │   ├── storage.py            # 磁盘对象存储
│   │   ├── rate_limit.py         # 限速
│   │   ├── worker.py             # 生命周期任务
│   │   └── routers/              # service_policy/save_sessions/templates/saves/retrievals
│   └── tests/
└── docs/
```
...
已选定的技术栈：前端 Vite + React + TypeScript（状态管理 zustand，路由 react-router-dom）；后端 FastAPI + SQLite（标准库 `sqlite3`）+ 本地磁盘对象存储；部署为单容器 Docker（FastAPI 托管前端产物）。浏览器姿态分析候选为 MediaPipe Face Landmarker（模型同源自托管、版本锁定），尚未完成遥测审计与模型 QA；对象存储与图片重编码（Pillow）只用于服务器暂存路径。云服务、托管数据库与 CDN 未选择，本地磁盘存储满足 MVP 部署范围。
