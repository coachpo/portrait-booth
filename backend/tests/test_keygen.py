"""KEY generation and normalization (SAV-003/004)."""
...
class Rng:
    """Controllable RNG: returns a fixed value sequence for rejection-sampling
...
def test_generate_key_uses_rejection_sampling_without_bias():
    # charset 36 → k=6, limit=64-64%36=36; randbelow(64) values 36..63 must be
    # rejected
    rng = Rng([36, 63, 0])  # 36, 63 out of range are rejected; 0 → charset[0]
    key = generate_key(rng=rng, settings=Settings())
    assert key == "AAAAAA"
    # First position rejects twice + one draw for each of the other 5
    assert rng.calls == 8
...
def test_generate_key_allows_all_letters_and_all_digits():
    ...
    rng = Rng([26] * 6)  # digits start at position 26
...
def test_generate_secret_is_at_least_128_bits():
    ...
    assert len(secret) >= 22  # 16 bytes base64url ≈ 22 chars


def test_normalize_key():
    assert normalize_key(" a7c 2f9 ") == "A7C2F9"
    assert normalize_key("a7c-2f9") == "A7C2F9"
    assert normalize_key("000000") == "000000"  # keep leading zeros
    assert normalize_key("a7c2f9") == "A7C2F9"  # lowercase to uppercase
